...
class Pose:
    ...
    vars = (sigmas * 2) ** 2
    last_id = -1
    color = [0, 224, 255]#BGR 黄色
    color_red=[30,0,255]#红色
    color_blue=[255,30,0]#蓝色
    ...
    def draw(self, img):
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1:
                ...
                #原来：cv2.circle(img, (int(x_a), int(y_a)), 3, Pose.color, -1)
                cv2.circle(img, (int(x_a), int(y_a)), 10, Pose.color_blue, -1)
            kpt_b_id = BODY_PARTS_KPT_IDS[part_id][1]
            global_kpt_b_id = self.keypoints[kpt_b_id, 0]
            if global_kpt_b_id != -1:
                x_b, y_b = self.keypoints[kpt_b_id]
                #原来：cv2.circle(img, (int(x_b), int(y_b)), 3, Pose.color, -1)
                cv2.circle(img, (int(x_b), int(y_b)), 10, Pose.color_blue, -1)
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), Pose.color_red, 8)
